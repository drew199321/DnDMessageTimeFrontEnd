...
function diceroll()
{
    ...
    var _temp = document.querySelectorAll("input[type=number][id=dice]");
    var temp = Array.from(_temp);
    var dice = []
    for (i in temp)
    {
        dice.push(temp[i].value);
        value.push(temp[i].getAttribute("name"));
    }
...
//The following code is for the dropdown menu with checkboxes inside of it.
var expanded = false;
...
var checkboxes = document.querySelectorAll("input[type=checkbox][class=check]");

checkboxes.forEach(function(checkbox) {
    checkbox.addEventListener("change", function() {
        var header = document.querySelector(".d" + this.id);
        var input = document.querySelector(".num" + this.id);
        if (this.checked) {
            header.innerHTML = "D" + this.id;
            input.innerHTML = "<input type = 'number' min='0' max='20' value='0' name='" + this.id + "' id='dice'>";
